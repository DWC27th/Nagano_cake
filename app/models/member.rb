class Member < ApplicationRecord
  ...
  validates :last_name, presence: true, format: { with: /\A(?:\p{Hiragana}|\p{Katakana}|[ー－]|[一-龠々])+\z/, message: "全角ひらがな・カタカナ・漢字で入力してください" }
  validates :last_name_kana, presence: true, format: { with: /\A[\p{katakana}\p{blank}ー－]+\z/, message: "は全角カタカナで入力して下さい。" }
  validates :first_name, presence: true, format: { with: /\A(?:\p{Hiragana}|\p{Katakana}|[ー－]|[一-龠々])+\z/, message: "全角ひらがな・カタカナ・漢字で入力してください" }
  ...
  validates :phone_number, presence: true, format: { with: /\A\d{10}\z/, message: "は10桁の整数で入力してください" }
